...
const STEPS: { n: string; title: string; text: string; hint?: string }[] = [
  {
    n: "01",
    title: "Укажите папку конфига",
    text: "Жмите «Папка» и выбирайте ~/.config/eww — инструмент сам подтянет все .yuck и .scss внутри, определит корневые eww.yuck и eww.scss и разложит вложенные файлы по include-путям. Вручную вставить содержимое или «Импорт» отдельных файлов тоже работает. А если хочется совсем без кликов — кнопка TUI в шапке: терминальная версия читает папку сама и обновляется на лету.",
    hint: "ls ~/.config/eww",
  },
  {
    n: "02",
    title: "Вложенные файлы — уже внутри",
    text: "При загрузке папки всё, что лежит внутри (например src/_volumes.yuck из (include \"src/_volumes.yuck\")), подключается автоматически. Докинуть файл отдельно можно кнопками «+ файл» или «вставить» (путь + содержимое). Анализатор проверит все файлы, разрешит пути include, найдёт циклы и дубли определений между файлами.",
